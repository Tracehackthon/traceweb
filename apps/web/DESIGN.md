...
- 个人 Web 的知乎／全网公开搜索已接入同源 Serverless 路由；原话先保存，搜索结果默认不入库，用户明确点击后才保留来源且不生成关系。普通网页不访问本机 Codex；桌宠的原生桥接可从本机 Runtime 读取 profile 并创建真实 Agent run，返回仍是未采纳候选。Trace 账号和云同步仍未接入。视觉高亮、关联或本地保存不能冒充结论采用。
...
`.demo-guide` 仅在 `/app/demo` 出现，默认收起，列出「留下一点、从这里接着、找个对照、我的理解、带去用、结果回来」六个动作及 6/6 完整状态。demo v3 用同一套产品命令建立 4 个 matters 和 2 个 works；首页四个气泡分别表现一份理解、一个工作承接、一次全新重读和一个待验证停点；六个直达入口都打开主事项的真实记录，不使用静态截图冒充可操作产品。来源弹层显示知乎开放平台摘要、作者、获取日期和安全的 HTTPS 原文入口，并明确摘要不是全文或用户判断。恢复演示只重建演示数据库，不触碰 `/app` 的个人数据。
...
| 不联网 / 知乎搜索 / 全网搜索 | 个人 Web 在原话保存成功后执行所选搜索；结果显示标题、作者、摘要、公开互动数据和原文链接，明确保留前不进入事项。demo 不发起访客搜索；其中两份来源来自预先完成的一次真实知乎开放平台查询。 |
| 「我的知乎内容」OAuth | 只在个人空间出现，按环境显示服务未配置、未连接或已连接；只有明确操作才读取，且不自动保存进 Trace。 |
| 不交给 Agent / Codex 原生 / Codex Harness / 自定义 Agent | Web 保存 handoff 意图；普通浏览器不执行。本机桌宠会按用户选择优先匹配服务端 profile，并通过 Product → Agent API 创建真实运行；结果保持未采纳候选。 |
...
- **Do** 在 composer 内用两个原生下拉区分搜索意图与 Agent 意图，并用「知乎参与这件事」来源条、个人 OAuth 实时状态和本地 handoff 说明守住边界。
