...
- `/app/demo` 和 `/app` 复用同一套真实产品组件与命令边界。托管 Web 为两者使用独立的 IndexedDB 命名空间；即使本地开发中的 `/app` 连接个人 SQLite，`/app/demo` 也固定使用独立演示 IndexedDB，不会把合成数据写入个人 SQLite。完整 demo v3 含 4 个 matters 与 2 个 works；其中主事项具有六动作可直达的完整链路。事项状态仍标记为 `synthetic`，但两份知乎来源是 2026-09-15 通过开放平台真实检索后缓存的公开摘要快照；首页明示「知乎来源快照 · 2 条 · 演示不联网」，个人空间不继承任何演示记录。
...
| 不联网 / 知乎搜索 / 全网搜索 | 下拉选择只保存本次搜索意图；当前 Web 不在访客会话发起联网请求或伪造结果。demo 明示「知乎来源快照 · 2 条 · 演示不联网」，所示来源来自预先完成的一次真实知乎开放平台查询。 |
...
- **Do** 在 composer 内用两个原生下拉区分搜索意图与 Agent 意图，并用「知乎来源快照 · 2 条 · 演示不联网」、个人 OAuth 实时状态和本地 handoff 说明守住边界。
