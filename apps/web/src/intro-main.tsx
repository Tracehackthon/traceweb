...
function AgentFocusCapture({ src, alt, focus }: { src: string; alt: string; focus: 'quiet' | 'input' | 'saved' | 'panel' | 'zhihu' }) {
  return <span className="agent-focus-capture" data-focus={focus}>
    <img className="capture-ambient" data-critical="true" src={src} alt={alt}/>
    <img className="capture-focus" src={src} alt="" aria-hidden="true"/>
    <span className="capture-focus-glow" aria-hidden="true"/>
  </span>;
}
...
function FormFactorJourney() {
  ...
  return <div className="format-journey">
    <figure className="format-preview" data-stage={stage.key}>
      ...
      <div className="preview-surface"><span className="preview-work">Trace 留在原来的工作现场</span>{stage.key === 'pet' || stage.key === 'input' || stage.key === 'bubble' ? <AgentFocusCapture src={stage.image} alt={stage.proof} focus={stage.key === 'pet' ? 'quiet' : stage.key === 'input' ? 'input' : 'saved'}/> : <img data-critical="true" src={stage.image} alt={stage.proof}/>}</div>
...
function ThinkingJourney() {
  ...
  return <div className="thinking-journey">
    ...
    <div className="thinking-panel" id="thinking-panel" role="tabpanel" data-stage={stage.key}><div className="thinking-copy"><p>{stage.kicker}</p><h3>{stage.title}</h3><div>{stage.body}</div>{stage.key === 'source' && <a href={zhihuSource} target="_blank" rel="noreferrer">查看知乎原文 <Icon name="external" size={15}/></a>}<small>{stage.proof}</small></div><div className="thinking-proof">{stage.image ? stage.key === 'source' ? <AgentFocusCapture src={stage.image} alt={stage.proof} focus="zhihu"/> : <img data-critical="true" src={stage.image} alt={stage.proof}/> : <ComparisonSample/>}</div></div>
  </div>;
}
