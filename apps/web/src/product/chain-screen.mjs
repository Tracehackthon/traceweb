...
const safeSourceURL = (source) => {
  try {
    ...
    const isZhihu = value.hostname === 'zhihu.com' || value.hostname.endsWith('.zhihu.com');
    return value.protocol === 'https:' && (source?.searchSource === 'global' || isZhihu) ? value.href : '';
  } catch { return ''; }
};
const sourceProvenance = (source) => source?.origin === 'provider-snapshot' && source?.provider === 'zhihu'
  ? `${source.searchSource === 'global' ? '全网搜索结果' : '知乎公开内容'}${source.author ? ` · ${source.author}` : ''}${source.fetchedAt ? ` · ${new Date(source.fetchedAt).toLocaleDateString('zh-CN')}` : ''}` : '';
const sourceMetrics = (source) => [Number.isSafeInteger(source?.voteUpCount) ? `${source.voteUpCount} 赞同` : '', Number.isSafeInteger(source?.commentCount) ? `${source.commentCount} 评论` : '', source?.contentType || ''].filter(Boolean).join(' · ');
const sourceProviderLabel = (source) => source?.searchSource === 'global' ? '全网公开内容' : '知乎公开内容';
...
export function mountChainScreen({root, view, onAction, onHome, onMatters, onBack, onWorkspaces, assets={}, services={}, demo=false}) {
  ...
  const sourceEvidence = (source, matter=current.matter) => sourceProvenance(source) ? `<button type="button" class="chain-source-evidence" data-action="source" data-source="${h(source.id)}"><i>${source?.searchSource==='global'?'网':'知'}</i><span><strong>${source?.searchSource==='global'?'全网来源':'知乎内容'}参与了这件事</strong><small>${h(source.author || (source?.searchSource==='global'?'公开来源':'知乎作者'))} · ${h(sourceRole(source,matter))}</small></span>${icon('next')}</button>` : btn('source','查看原现场','file','subtle',source?`data-source="${h(source.id)}"`:'');
  ...
  function handoff() {
    ...
    const binding = current.nativeProjectBinding || {};
    const bindingStatus = binding.status === 'confirmed' ? '已确认当前项目' : binding.status === 'candidate' ? '发现项目候选 · 需先确认' : binding.status === 'conflict-or-drift' ? '项目身份发生变化 · 需重新选择' : '尚未确认项目绑定';
    const bindingAction = binding.status === 'confirmed' ? '交给 Codex' : '保存工作准备';
    return `<main class="chain-handoff-layout" data-key="main"><section class="chain-handoff-context"><div class="chain-heading"><h1>我的理解<b>。</b></h1><p>个人草稿 · ${m.understandingDraft===m.understanding&&m.understanding?'已保存':'草稿 · 尚未确认为当前理解'}</p></div><div class="chain-glass chain-handoff-summary">${row('',m.understanding || '暂无已保存的理解，可以直接写。')}${row('还没分清',m.stop || '尚未留下停点。','stop')}${current.incoming?.text ? `<div class="chain-inference">${icon('bulb')}这次带来：${h(current.incoming.text)}</div>`:''}</div>${composer('先写下这次准备怎样尝试……')}</section><section class="chain-handoff-side chain-glass"><div class="chain-side-heading"><h1>带去这次工作</h1>${btn('back','关闭','close','icon-only')}</div><div class="chain-panel-scroll"><section class="chain-destination-summary" aria-label="工作去向"><span class="chain-native-mark">C›_</span><div><strong>${h(dest.agent||'Codex')}</strong><span>${bindingAction === '交给 Codex' ? bindingStatus : `${bindingStatus}；确认后才能交给 Codex`}</span></div><small>项目来源和身份状态以桌宠设置为准；路径、登录信息和系统参数不会显示在页面。</small></section><label class="chain-label">这次请它做什么${smallInput('task',dest.task,'例如：核对条件并给出可复核结果')}</label><label class="chain-label">选中的内容${field('handoff-text',ho.selectedText,'选择这次实际要带入的文字','chain-handoff-text','rows="3"')}<small>来源：${h(title())}</small></label><fieldset class="chain-role"><legend>我希望这次</legend>${[['trial','这次准备试','pen'],['reference','作为参考','file'],['exclude','这次不用','no']].map(([key,label,name])=>btn('role',label,name,ho.role===key?'selected':'',`data-role="${key}" aria-pressed="${ho.role===key}"`)).join('')}</fieldset><label class="chain-label">补充说明（可选）${field('handoff-note',ho.note,'想观察什么，或不希望怎样使用？','chain-handoff-note','rows="2"')}</label><label class="chain-label">使用范围<select class="chain-field" data-field="scope" aria-label="使用范围"><option value="current-task">只用于这次工作</option></select><small>不会自动变成长期要求。</small></label></div><footer class="chain-handoff-actions">${btn('confirm-handoff',binding.status === 'confirmed' && /^Codex(?:\s|$|原生)/i.test(dest.agent||'Codex')?'交给 Codex':'保存工作准备',null,'primary',ho.selectedText?.trim() && ho.role!=='exclude' && dest.task?.trim()?'':'disabled')}${btn('exclude-handoff','先不带入',null)}</footer></section></main>`;
  }
  function work() {
    const ho=current.handoffSnapshot || current.handoff || {},dest=ho.destination||{};
    return `<main class="chain-work-layout" data-key="main"><section class="chain-native-example"><header><span class="chain-native-mark">C›_</span><strong>${h(dest.agent||'未选择 Agent')} · ${h(dest.project||'未选择项目')}</strong><span class="chain-example-badge">${isDemo()?'演示中的工作':'本次工作准备'} · 尚未交给 Agent</span></header><div class="chain-native-body"><h1>${h(dest.task||'未选择任务')}</h1><p>继续在原来的 Agent 中工作</p><div class="chain-native-explanation">${icon('layers')}<div><h2>这里保留本次准备与工作中的发现。</h2><p>Trace 不增加第二个工作聊天；只有确认交接后，内容才会交给 Agent。</p></div></div><div class="chain-native-artifact"><span class="chain-round">${icon('file')}</span><div><h2>本次准备带入</h2><p>${h(ho.selectedText || '尚未带入本次工作')}</p><small>理解版本：${h(ho.understandingVersion ?? 0)} · ${ho.role==='reference'?'作为参考':'这次准备试'}</small></div></div><p class="chain-footnote">完成工作后，再把实际结果带回来判断。</p></div></section><aside class="chain-work-side chain-glass"><div class="chain-side-heading"><span class="chain-small-brand"><span class="brand-tile">${mark}</span>Trace</span>${btn('resume','回到这件事','back','icon-only')}</div><p class="chain-muted">${h(dest.agent||'未选择 Agent')} · ${h(dest.project||'未选择项目')} / ${h(dest.task||'未选择任务')}</p><div class="chain-panel-scroll"><h2 class="chain-section-heading"><span class="chain-round">${icon('file')}</span>本次带入</h2><section class="chain-work-card"><h2>${h(ho.selectedText || '尚未带入本次工作')}</h2><p>${ho.role==='reference'?'作为参考':'这次准备试'}</p><div class="chain-status-line"><span>${icon('check')}${current.handoff?.confirmed?'已确认 · 仅本次任务':'尚未带入本次工作'}</span><span class="chain-amber-label">${(current.matter?.results||[]).length ? '已有带回的结果' : '结果还没有回来'}</span></div><div class="chain-two-actions">${btn('provenance','查看依据','file')}${btn('exclude-handoff','这次不用','no')}</div></section><h2 class="chain-section-heading"><span class="chain-round">${icon('bulb')}</span>工作中新增</h2><section class="chain-work-card">${(current.workFindings||[]).map(f=>`<p>${h(f.text||f)}</p>`).join('')}${field('finding',findingDraft,'写下本次工作里的新发现……','chain-finding-input','rows="3"')}<div class="chain-two-actions">${btn('save-finding','先留一下','bookmark','',findingDraft.trim()?'':'disabled')}${btn('expand-finding','展开想想','pen','',findingDraft.trim()?'':'disabled')}</div></section></div><footer>${btn('results','带回结果','arrow','primary')}${btn('resume',title(),'file','bare')}</footer></aside></main>`;
  }
  function results() {
  ...
  function modalHTML() {
    ...
    if (modal.type==='source') { const s=getSource(modal.id)||firstSource(),url=safeSourceURL(s),provenance=sourceProvenance(s),provider=Boolean(provenance),label=sourceProviderLabel(s),metrics=sourceMetrics(s); heading=provider?`${label}摘录`:'原文片段'; body=`${provider?`<div class="chain-source-identity"><i>${s?.searchSource==='global'?'网':'知'}</i><span><strong>${h(label)}</strong><small>${h(s.author || (s?.searchSource==='global'?'公开来源':'知乎作者'))} · ${h(sourceRole(s,m))}</small></span></div><p class="chain-source-provenance">${h(provenance)}${metrics?` · ${h(metrics)}`:''}</p>`:''}<h3>${h(s?.title || '暂无来源')}</h3><blockquote>${h(s?.excerpt || m.originalText || '')}</blockquote>${url?`<a class="chain-source-link" href="${h(url)}" target="_blank" rel="noreferrer">${s?.searchSource==='global'?'打开原文':'打开知乎原文'} ${icon('external')}</a>`:''}<p class="chain-footnote">${provider?'这里展示的是公开摘要，不是完整原文；保留材料也不会改写“我的理解”。':'保留这份材料不会改写“我的理解”。'}</p>`; }
  ...
  function render() {
    ...
    fresh.innerHTML=`${!['reading','work'].includes(current.screen)?header():''}${scenery()}${renderer()}<footer class="chain-prototype-label" data-key="truth">${isDemo()?'完整演示 · 含已保存的知乎公开内容 · 与个人空间分开':''}</footer>${current.notice && current.screen!=='paused' ? `<div class="chain-notice" data-key="notice" role="status"><span>${h(current.notice)}</span>${btn('clear-notice','关闭','close','icon-only')}</div>`:''}`;
