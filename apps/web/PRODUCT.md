...
## Scope
2026-09-16 的产品路由保持一致：访问 `/` 先理解产品，再由明确入口进入 `/app/demo` 的完整浏览器演示或 `/app` 的个人空间；`/video` 播放由真实 Trace 桌面端、桌宠、知乎公开搜索与 Local Codex 回流画面制作的产品视频。同一 Vercel 部署承载产品介绍、产品应用、视频页和最小同源知乎授权接口；托管 Web 按 origin 使用 IndexedDB，本机宿主仍可接 SQLite，原 Electron 讨论入口保留为 legacy。
