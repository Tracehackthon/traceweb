...
本机 Runtime 会把状态保存在应用数据目录。桌宠位置也保存在同一份 `desktop-settings.json` 的版本化 `placement` 节点中，按 `displayId` 和分辨率保留记录；换显示器或升级版本时优先恢复对应显示器，再回退到同分辨率记录并做工作区边界校正，不会因为窗口版本更新而清空位置。知乎公开搜索和个人账号授权连接 `https://trace.neutrom.store` 的同源服务，不会把 App Secret 打进安装包。点「授权连接知乎」后，Trace 会打开一个独立的知乎授权窗口；成功回到 Trace 后窗口自动关闭，Token 只保存在桌面应用的加密 HttpOnly Cookie 中。服务不可用时界面只显示清楚的用户提示，不暴露内部接口字段。
...
产物位于 `release/Trace-Desktop-<version>-Setup.exe`。0.2.0 尚未进行 Windows 代码签名，系统可能显示未知发布者提示。
...
npm run release:smoke -- --unpacked apps/desktop-pet/release/win-unpacked --expected-version 0.2.0

# 输出安装包或目录的 SHA-256 清单；目录摘要按稳定路径顺序计算
npm run release:checksum -- --artifact apps/desktop-pet/release/Trace-Desktop-0.2.0-Setup.exe
...
运行后，刘看山以透明、无边框、置顶的桌面 Overlay 出现在当前绑定显示器工作区。透明区域不阻挡其他应用；点刘看山后，卡片与面板在当前桌面展开。空闲时大部分区域保持 click-through，宠物、提醒、卡片和面板 hover/drag/open 时才交给原生窗口处理；隐藏窗口则完全穿透。系统托盘菜单可以显示、隐藏或退出插件。
...
原生桌宠不是另一个 Agent。主进程从已经连接的 Trace Runtime/Codex host 读取安全的 turn 投影，通过 `trace-native:codex-event` IPC 发给 Renderer；Renderer 只运行 reducer，把它投影为 `idle / thinking / running-command / editing / searching / waiting / review / failed` 等视觉状态。事件身份按 `threadId + turnId + itemId + 阶段` 去重，完成/失败/取消等终态不经过延迟合并，因此不会因为 debounce 丢掉终态。

当前适配器优先读取 Runtime 已有的 host turns 列表；只有带稳定 thread/turn 身份的行才会进入桌宠，字段会在桥接层截断并过滤绝对路径、命令正文和常见 token。若 Runtime 尚未提供 item 级事件，桌宠保持安静，不用随机/mock 事件冒充真实 Codex 进度；后续可以让 Codex app-server adapter 输出同一 `TraceCodexEvent` 合约而不改 UI/reducer。Web profile 仍保留原有 mock reminder，仅用于演示，不代表 Codex 运行状态。

原生 overlay 上报交互 bounds 作为跨平台 input-shape 的可观测骨架，当前以 `setIgnoreMouseEvents` + `elementFromPoint` fallback 工作；不会复制 Codex Desktop 的闭源 UI、素材或 OpenAI mascot。
...
桌宠观察列表本身仍是进程内会话状态；Codex 事件 reducer 与显示器位置骨架已加入，但 item 级 app-server adapter、模拟语音、图片识别文案、图谱拖拽缩放、安装包签名与自动更新尚未实现。后端产品事项、Codex 交付和 Agent run 由 `trace-runtime` 持久化，但返回结果不会伪装成已经修改用户理解。当前原生入口先验证 Windows 桌面交互，尚未完成 macOS / Linux 验证，也没有接管系统开机启动。
