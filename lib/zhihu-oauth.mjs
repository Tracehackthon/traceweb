...
export const REDIRECT_URI = process.env.ZHIHU_OAUTH_REDIRECT_URI || 'https://trace.neutrom.store/callback';
const APP_KEY = process.env.ZHIHU_OAUTH_APP_KEY || '';
export const ACCESS_SECRET = process.env.ZHIHU_ACCESS_SECRET || '';
const CONFIGURED_ORIGINS = String(process.env.TRACE_ALLOWED_ORIGINS || 'https://traceweb-neutronm.vercel.app')
  .split(',')
...
export function requireSameOrigin(req, res) {
  ...
  const allowed = new Set(expected ? [expected] : []);
  for (const value of [REDIRECT_URI, ...CONFIGURED_ORIGINS]) {
    try {
      const url = new URL(value);
      if (url.protocol === 'https:' && !url.username && !url.password) allowed.add(url.origin);
    } catch { /* Invalid configuration is ignored and never broadens access. */ }
  }
